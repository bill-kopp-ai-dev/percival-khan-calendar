...
def test_view_agenda_happy(view_app, monkeypatch, isolated_workspace):
    monkeypatch.setattr(
        ...
        lambda *a, **kw: KhalResult(
            stdout="10:00 Standup", returncode=0, elapsed_ms=1
        ),
    )
...
def test_view_agenda_empty_returns_msg(
    view_app, monkeypatch, isolated_workspace
):
    monkeypatch.setattr(
        "percival_khan_calendar.adapters.subprocess_runner.subprocess.run",
...
def test_view_calendar_long_truncates(
    view_app, monkeypatch, isolated_workspace
):
    big = "x" * 5000
    monkeypatch.setattr(
